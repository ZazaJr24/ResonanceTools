...
public sealed class GoldbergViewModel : ViewModelBase
{
    ...
    private static readonly string EmuRoot = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ResonanceTools", "gbe_fork");
    ...
    private static readonly string OptionsFile = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ResonanceTools", "goldberg_options.json");
        ...
        : base(store, navigation, logging)
    {
        ...
        _http.DefaultRequestHeaders.UserAgent.ParseAdd("ResonanceTools/1.0");
