...
{
    private readonly ISteamCatalogService _catalog; private readonly IRyuuCatalogService _ryuu; private readonly ILibrarySyncService _librarySync;    private string _search=""; private string _sort="Popular (AAA)"; private string _typeFilter="All games"; private string _source=RyuuSource; private int _page=1; private int _pageSize=48; private LibraryNsfwScope _nsfwScope=LibraryNsfwScope.Hide;
    public const string RyuuSource="Available (Ryuu)";
    public LibraryViewModel(IAppDataStore s, INavigationService n, ILoggingService l, ISteamCatalogService c, IRyuuCatalogService ryuu, IArtworkService _, ILibrarySyncService sync) : base(s,n,l) { _catalog=c; _ryuu=ryuu; _librarySync=sync; }
    ...
    public string[] Sources { get; }={"Steam catalog", RyuuSource}; public string SelectedSource { get=>_source; set { if(SetProperty(ref _source,value)){ _page=1; CatalogLoaded=false; OnPropertyChanged(nameof(CatalogCountLabel)); _ = LoadAsync(true); } } } public bool IsRyuuSource=>_source==RyuuSource;
