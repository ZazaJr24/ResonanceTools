...
public sealed class DownloadJob : UiObservableObject
{
    ...
    private const int MaxProcessLogLength = 120_000;

    public void AppendLog(string line)
    {
        if (string.IsNullOrWhiteSpace(line)) return;

        var separator = string.IsNullOrEmpty(_processLog) ? string.Empty : Environment.NewLine;
        if (line.Length >= MaxProcessLogLength)
        {
            ProcessLog = line[^MaxProcessLogLength..];
            return;
        }

        var retainedLogLength = Math.Max(0, MaxProcessLogLength - line.Length - separator.Length);
        var retainedLogStart = Math.Max(0, _processLog.Length - retainedLogLength);
        ProcessLog = string.Concat(_processLog[retainedLogStart..], separator, line);
    }
}
